...
**Tech Stack:** Node 22.13, TypeScript 7.0.2, Nx 23.2.1, NestJS 12.0.3 (Fastify), Prisma 7.10.0, PostgreSQL 17, NATS JetStream 2.10, Redis 7, Zod 4.6.5, Vitest 5.0.1, Pino 10.3.1, Testcontainers 12.1.0, Docker Compose.
...
- **Exact version pins.** Prisma CLI and `@prisma/client` are both pinned to `7.10.0` — `prisma@latest` currently resolves to an `8.0.0-rc` and must never be used.
