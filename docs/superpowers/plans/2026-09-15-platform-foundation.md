...
- **Each Prisma service sets its own generator `output`.** With no `output`, pnpm dedupes services sharing an identical `@prisma/client` graph into one physical `node_modules/.prisma/client`, and generating one service's client silently overwrites another's. Scope it per app (`node_modules/.prisma-client-<service>`). Verify by generating two services in sequence and running both suites — that ordering is the actual regression.
- **A composite unique over nullable columns does not prevent duplicates in PostgreSQL**, because `NULL` is never equal to `NULL` in a unique index. `@@unique([userId, roleId, projectId, siteId])` therefore constrains nothing for global (`null`, `null`) or project-scoped (`projectId`, `null`) assignments — the two most common shapes. Express these as partial unique indexes in raw migration SQL, one per NULL-shape, since Prisma's schema language cannot declare them.
- **`nx test a b` forwards the second project name to vitest as a file filter** and fails with "No test files found". Use `nx run-many -t test -p a,b` whenever targeting more than one project.
- **Every service verifies the JWT itself; the gateway is not trusted to vouch for a principal.** The gateway proxies a *new* HTTP request upstream, so `req.user` does not survive the hop and `@RequirePermission` is inert metadata without it. `@ipms/authz` exports `JwtUserGuard`, which verifies the HS256 signature and `exp` and populates `req.user`. Every service registers it as an `APP_GUARD` **before** `AuthzGuard` (registration order is execution order) and provides a `SCOPE_PROVIDER`. Health and metrics endpoints carry `@Public()`. Revocation — the Redis `tokenVersion` check — stays at the gateway alone; a service honouring a token revoked within its 15-minute TTL is the accepted exposure, and putting Redis in every service to close it is not worth the coupling.
- **Nest DI cannot see a constructor parameter typed with `import type`.** TypeScript erases it, `emitDecoratorMetadata` records `Object`, and Nest fails at bootstrap trying to resolve an `Object` token. This bit `AuthzGuard` (its `Reflector`), `ChainService` and `AuditController` (their `PrismaClient`). Use a value import plus `@Inject(Token)`, and note that unit tests constructing a class directly will never catch it — only a real boot does. Tasks 19 and 21 must boot their services as a verification step for this reason.
- **Prisma 7 wiring differs from Prisma 6 and from most published examples.** Two breaking changes, verified against the pinned `7.10.0`:
  1. `datasource { url = env("DATABASE_URL") }` in `schema.prisma` is rejected. The URL moves to a `prisma.config.ts` beside the schema: `defineConfig({ schema, migrations: { path }, datasource: { url: env('DATABASE_URL') } })`. The `datasource` block keeps only `provider`.
  2. `new PrismaClient({ datasources: { db: { url } } })` no longer connects anything. Prisma 7 requires a driver adapter: `new PrismaClient({ adapter: new PrismaPg({ connectionString }) })`, with `@prisma/adapter-pg` and `pg` as dependencies.
  Each service wraps this in its own `PrismaService extends PrismaBaseService`, reading its own `<SERVICE>_DATABASE_URL`. `DATABASE_URL` is reserved for the Prisma CLI's migration tooling. `apps/audit/src/prisma.service.ts` and `apps/audit/prisma.config.ts` are the working reference — copy that shape in Tasks 14, 15 and 19 rather than the `datasources` form written in their code blocks.
- **The audit ledger's `actorId` is `@db.VarChar(100)`, not `@db.Uuid`.** The ledger is a record-of-fact sink: an audit write that fails validation loses the record, which is worse than storing an unexpected string. This is deliberately the opposite trade from request DTOs, where `UuidSchema` rejects non-v7 input at the boundary. `objectId` and `objectType` are `VarChar` for the same polymorphic reason.
