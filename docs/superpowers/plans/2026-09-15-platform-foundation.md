...
- **No service writes to the audit ledger directly.** Services publish to `audit.event`; only `audit` writes audit rows.
...
│   │   ├── src/payloads/audit.ts    audit.event payload type
...
describe('subject catalog', () => {
  ...
  it('routes the audit stream to exactly one durable consumer', () => {
    expect(STREAMS.AUDIT.subjects).toEqual(['audit.event']);
...
- Produces:
  - `AuditConsumer` — the single durable subscriber to `audit.event`
...
describe('RolesService.create', () => {
  ...
  it('writes an audit event in the same transaction', async () => {
    ...
    expect(tx.outboxEvent.create).toHaveBeenCalledWith(
      expect.objectContaining({ data: expect.objectContaining({ subject: 'audit.event' }) }),
    );
  });
});
...
describe('ScopesService.grantProject', () => {
  ...
  it('also emits an audit event', async () => {
    ...
    await service.grantProject(USER, PROJECT, ACTOR);
    const subjects = tx.outboxEvent.create.mock.calls.map((c) => c[0].data.subject);
    expect(subjects).toContain('audit.event');
  });
...
const pending = [
  ...
  { id: 'e-2', subject: 'audit.event', payload: { action: 'role.created' }, correlationId: 'c-1', actorId: 'a-1' },
];
